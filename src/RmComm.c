...
static uint8_t u08_RM_BaseCycle;
static uint32_t u32_RM_Password;
...
static uint8_t u08_CommPayloadSize;
    ...
    {
        u16_RM_RcvTimeoutCnt = u16_RM_RcvTimeoutCnt + u08_RM_BaseCycle;
...
static uint8_t CtrlSendTiming( uint8_t u08_mode, uint8_t u08_trg )
{
    ...
    if( u08_mode != RM_LOG_MODE )
    {
        ...
        u16_RM_WaitCnt = u08_RM_BaseCycle;
    ...
    else
    {
        ...
        if( u16_RM_ReqTimeoutCnt >= RM_REQ_TIMEOUT_CNT )
        {
            ...
            u16_RM_TimingCnt = RM_SND_DEFAULT_CNT;
        ...
        else
        {
            ...
            u16_RM_ReqTimeoutCnt = u16_RM_ReqTimeoutCnt + u08_RM_BaseCycle;
            ...
            {
                u16_RM_WaitCnt = u16_RM_WaitCnt + u08_RM_BaseCycle;
...
static uint16_t RequestDUTCondition( uint8_t u08_tmp[], uint8_t u08_payloadsize )
{
    ...
    else
    {
        ...
        if( u32_password != u32_RM_Password )
        {
            u16_return_val = RM_STATUS_ERR;
...
void RM_Initial( uint8_t u08_tmp[], uint8_t u08_buff_size, uint8_t u08_base_cycle, uint32_t u32_password )
{
    ...
    if( u08_base_cycle > 0 )
    {
        u08_RM_BaseCycle = u08_base_cycle;
    }
    else
    {
        u08_RM_BaseCycle = RM_DEFAULT_BASE_CYCLE;

    }

    u32_RM_Password = u32_password;
